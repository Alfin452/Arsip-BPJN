<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="font-bold text-3xl text-slate-900 dark:text-white leading-tight tracking-tight">
                    {{ __('Edit Dokumen SP2D') }} - <span class="text-blue-600 dark:text-blue-400">{{ $sp2d->nomor_sp2d }}</span>
                </h2>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Perbarui informasi atau lampiran dokumen SP2D.</p>
            </div>
    ...
    <div class="py-10" x-data="sp2dForm()">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl p-5 flex items-start gap-4 shadow-sm">
                <div class="bg-red-100 dark:bg-red-800/50 p-2 rounded-xl text-red-600 dark:text-red-400 shrink-0">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                </div>
                <div>
                    <h4 class="font-bold text-red-800 dark:text-red-400 text-base">Dokumen ini ditolak oleh Verifikator</h4>
                    <p class="text-sm text-red-600 dark:text-red-300 mt-1">Silakan perbaiki data di bawah dan ajukan kembali.</p>
                </div>
            </div>
            @endif
            ...
            <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-[0_2px_10px_rgba(0,0,0,0.04)] dark:shadow-none border border-slate-100 dark:border-slate-700 overflow-hidden">
                <form action="{{ route('sp2d.update', $sp2d->id) }}" method="POST" enctype="multipart/form-data" class="form-save relative">
                    @csrf
                    @method('PUT')
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                                    ...
                                    <select name="spm_id" id="spm_id" x-model="selectedSpm" @change="updateSpmInfo()" class="block w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm" required>
                                        ...
                                        @foreach($spms as $spm)
                                            <option value="{{ $spm->id }}" 
                                                    ...
                                                    @if(old('spm_id', $sp2d->spm_id) == $spm->id) selected @endif>
                                                {{ $spm->nomor_spm }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="nomor_sp2d" value="{{ old('nomor_sp2d', $sp2d->nomor_sp2d) }}" class="block w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm font-medium uppercase" required>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input x-init="flatpickr($el, { dateFormat: 'Y-m-d' })" type="text" name="tanggal_sp2d" value="{{ old('tanggal_sp2d', \Carbon\Carbon::parse($sp2d->tanggal_sp2d)->format('Y-m-d')) }}" class="block w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm cursor-pointer" required>
                                </div>

                                <!-- Nilai SP2D -->
                                <div x-data="{ 
                                    rawValue: '{{ old('nilai_sp2d', $sp2d->nilai_sp2d) }}',
                                    formattedValue: '{{ old('nilai_sp2d', $sp2d->nilai_sp2d) ? number_format(old('nilai_sp2d', $sp2d->nilai_sp2d), 0, '', '') : '' }}',
                                    init() {
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="text" x-model="formattedValue" @input="formatRupiah" class="block w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm font-mono text-lg font-bold" required>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea name="keterangan" rows="3" class="block w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm">{{ old('keterangan', $sp2d->keterangan) }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="border-t border-slate-100 dark:border-slate-700 pt-10">
                            <div class="flex items-center gap-3 mb-6">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-slate-500 dark:text-slate-400">Biarkan kosong jika tidak ingin mengubah dokumen SP2D.</p>
                                </div>
                            </div>
                            ...
                            <div class="bg-blue-50/50 dark:bg-blue-900/10 p-5 rounded-2xl border-2 border-blue-100 dark:border-blue-800/50 relative overflow-hidden group hover:border-blue-300 dark:hover:border-blue-700 transition-colors">
                                ...
                                <div class="flex justify-between items-start mb-3 relative z-10">
                                    <label class="block text-sm font-bold text-blue-900 dark:text-blue-300">Dokumen SP2D PDF</label>
                                    @if($sp2d->file_pdf)
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('sp2dForm', () => ({
                selectedSpm: '{{ old("spm_id", $sp2d->spm_id) }}',
                nilaiSp2d: '{{ old("nilai_sp2d", $sp2d->nilai_sp2d) }}',
                spmInfo: {
                    satker: '',
                    ppk: '',
                    nilai: 0
