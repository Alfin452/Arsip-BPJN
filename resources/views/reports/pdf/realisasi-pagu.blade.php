...
<head>
    <meta charset="utf-8">
    <title>Laporan Realisasi Pagu</title>
    <style>
...
<body>
    ...
    <div class="report-title">LAPORAN REALISASI PAGU ANGGARAN (DIPA)</div>
    <div class="report-subtitle">Tahun Anggaran {{ $tahun }}</div>

    <table class="data-table">
        <thead>
            <tr>
                <th class="text-left">Satuan Kerja (Satker)</th>
                <th class="text-right">Pagu Anggaran (Rp)</th>
        ...
        <tbody>
            ...
            <tr>
                <td class="text-left"><strong>{{ $row['satker'] }}</strong></td>
                <td class="text-right">{{ number_format($row['pagu'], 0, ',', '.') }}</td>
                <td class="text-right highlight">{{ number_format($row['realisasi'], 0, ',', '.') }}</td>
            ...
            @empty
            <tr>
                <td colspan="4" class="text-center">Belum ada data transaksi untuk tahun {{ $tahun }}.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
