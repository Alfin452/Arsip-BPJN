...
<head>
    <meta charset="utf-8">
    <title>Laporan Distribusi Proyek per Penyedia Jasa</title>
    <style>
...
<body>
    ...
    <div class="report-title">Laporan Distribusi Proyek per Penyedia Jasa</div>
    <div class="report-subtitle">Tahun Anggaran {{ $tahun }}</div>

    <table class="data-table">
        <thead>
            <tr>
                <th class="text-left">Nama Penyedia Jasa</th>
                <th class="text-center">Jumlah Paket</th>
                <th class="text-right">Total Nilai Kontrak (Rp)</th>
            </tr>
        </thead>
        <tbody>
@php $grandTotal = 0; @endphp
            @forelse($tableData as $row)
            @php $grandTotal += $row->total_kontrak; @endphp
            <tr>
                <td class="text-left"><strong>{{ $row->nama_perusahaan }}</strong></td>
                <td class="text-center">{{ $row->jumlah_paket }}</td>
                <td class="text-right">{{ number_format($row->total_kontrak, 0, ',', '.') }}</td>
            </tr>
            @empty
            <tr>
